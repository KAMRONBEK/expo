...
describe(runAdbDeviceQueryAsync, () => {
  ...
  it('maps wait-policy expiry after observing graceful cleanup', async () => {
    ...
    const result = runAdbHostQueryAsync(
      'adb',
      ['devices'],
      'device discovery',
...
describe('subprocess cleanup integration', () => {
  ...
  it('drains both pipes, escalates where supported, and reaps after cancellation', async () => {
    const error = await runFixture(AbortSignal.timeout(300));

    expect(error).toBeInstanceOf(AdbProcessWaitError);
  });
});

async function runFixture(signal: AbortSignal): Promise<AdbProcessError> {
  ...
  try {
    ...
    expect(error).toBeInstanceOf(AdbProcessError);
    const pid = Number(fs.readFileSync(pidFile, 'utf8'));
    expect(Number.isInteger(pid)).toBe(true);
    expect(() => process.kill(pid, 0)).toThrow();
    return error as AdbProcessError;
  } finally {
